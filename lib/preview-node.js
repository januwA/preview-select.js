...
class PreviewNode {
    ...
    preview() {
        ...
        if (utils_1.getPV(target, "position") === "static") {
        ...
        if (preview_select_1.PreviewSelect.toStyle) {
            for (const key in preview_select_1.PreviewSelect.toStyle) {
                ...
                if (_isNaN && Object.getOwnPropertyDescriptor(target.style, key)) {
                    const value = preview_select_1.PreviewSelect.toStyle[key];
                    if (value) {
                        // transform是多参数的属性，避免冲突内部动画
                        if (key === "transform") {
                            target.style[key] = `translate(${x}px , ${y}px) ${preview_select_1.PreviewSelect.toStyle[key]}`;
                        }
                        else {
                            target.style[key] = preview_select_1.PreviewSelect.toStyle[key];
                        }
                    }
                }
            }
